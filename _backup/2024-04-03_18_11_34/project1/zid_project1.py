...
def read_dat(tic):
    ...
    # <COMPLETE THIS PART>
    ticpath = os.path.join(DATDIR, f"{tic}_prc.dat")
    with open(ticpath, 'r') as file:
...
def line_to_dict(line):
    ...
    for column in COLUMNS:
        end_index = start_index + COLWIDTHS[column]
        dictionary[column] = line[start_index:end_index]
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    verify_cols(col_lst)
    tickers_lst = tickers_lst or tic_exchange_dic.keys()
    col_lst = col_lst or COLUMNS
    data_dict = {}

    for ticker in tickers_lst:
        ticker_data = read_dat(ticker)
        filtered_data = [
            {col: line[col] for col in col_lst} for line in map(line_to_dict, ticker_data)
        ]
        data_dict[ticker] = {
...
def create_json(data_dict, pth):
    ...
    """
    # <COMPLETE THIS PART>
    with open(pth,'w') as opfl:
        json.dump(data_dict,opfl)
    opfl.close()
